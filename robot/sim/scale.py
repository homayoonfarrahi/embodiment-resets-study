...
class RunningScale(torch.nn.Module):
    ...
    def __init__(self, tau=0.01, device='cpu'):
        super().__init__()
        self.tau = tau
        # self.value = torch.ones(1, dtype=torch.float32, device=device)
    ...
    def _positions(self, x_shape):
        ...
        ceiled = floored + 1
        ceiled = torch.where(ceiled > x_shape - 1, torch.ones_like(ceiled) * (x_shape - 1), ceiled)
    ...
    def update(self, x):
        ...
        # value = percentiles[1] - percentiles[0]
